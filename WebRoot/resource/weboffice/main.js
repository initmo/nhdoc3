...
function SaveDoc(id,docType) {
	try{
		...
		returnValue = webObj.HttpPost("http://127.0.0.1:8080/nhdoc/kc/FileLoadAction.17?method=upLoadFiles&parentId=1&menuId=1");
		if("succeed" == returnValue){
			alert(returnValue+"文件上传成功");	
		}else if("failed" == returnValue)
			alert(returnValue+"文件上传失败");
		return_onclick(); 
	}catch(e){
		alert("异常\r\nError:"+e+"\r\nError Code:"+e.number+"\r\nError Des:"+e.description);
	}
...
*
/****************************************************/
function bToolBar_FullScreen_onclick(isFullScreen) {
	try{
		var webObj=document.getElementById("WebOffice1");
		webObj.FullScreen = isFullScreen;
	}catch(e){
		alert("异常\r\nError:"+e+"\r\nError Code:"+e.number+"\r\nError Des:"+e.description);
	}
}
